...
Wow, you made it this far, have a <Cookie name="testing" /> before we continue with more practical tips.
...
We can write a lot safe-guards and unit tests to validate the data we receive is of `MyType` and otherwise it should throw an error.
...
We could also use [`zod`](https://zod.dev) to validate the data we receive.
...
Writing clean tests can become tedious when you have to write a lot of boilerplate code just to pass in some variables/objects to a method.
...
But sometimes we just need _to force something into a space_. [`@total-typescript/shoehorn`](https://www.npmjs.com/package/@total-typescript/shoehorn) is the tool you are looking for. It will allow you to pass partial data in tests while keeping TypeScript happy.
...
Some of you might be familiar with [`storybook`](https://storybook.js.org/docs/writing-tests/test-runner), [`cypress`](https://www.cypress.io) or [`playwright`](https://playwright.dev).

These tools are powerfull in their own right, but I think they are bloatware when writing unit tests.

[`@testing-library`](https://testing-library.com) are _"simple and complete testing utilities that encourage good testing practices"_.
...
[codecov](https://about.codecov.io) will give you these insights and you can use it to make educated decisions on the state of your codebase.

## Conducting simulations

The beaty of testing is that you can simulate (mock) almost everything. This allows you to put your system under stress and force it into states that are hard to reproduce in real life.
