...
description: "And the art of the bodge"
pubDate: "Feb 21 2024"
headerColor: "#EBA6A9"
...
---
---
![AuraMotions](/blog/talking-to-water/auramotions.jpeg "AuraMotions, picture by ???")

---

> “What we say to water can impact its crystals: positive words create intricate structures and negative words lead to collapse (Dr. Masaru Emoto). <br/><br/>AuraMotions is an art project, that processes what we say to make different colours and patterns of water. <br/><br/> It applies sentiment analysis technology to detect emotions from what we say. Then, through MQTT, data is sent to TouchDesigner to create captivating effects on water.”
> <cite>The student group[^1]</cite>

---

In this post I would like to share some light the technical aspects of this project.

I will keep it light and easy to follow, leaving out jargon until going [down the rabbit hole](#down-the-rabbit-hole).

For the non-technical aspects, I would like to refer you to the students[^1] themselves.

[^1]: The students are: sdf asdkjfh asdljkf j
...
## The game plan
In order to _talk to water_ I broke the problem down in 3 steps:
...
You are able to use the Whisper API to convert speech to text. But some cool folks have build a [python wrapper](https://github.com/abdeladim-s/pywhispercpp) around the open-source model for easy, free and local use.
...
At the university we happen to have set-up a [MQTT broker](https://mosquitto.org) for students to do just that.
...
client = mqtt.Client("talk-with-water" + str(random.randint(0, 1000)))
...
### Works on my machine
While the code presented above worked on my machine, and might work on your machine with some technical knowledge, the bodged solution is not without its flaws.
...
This is a common problem in software development. Make a `Dockerfile` and ship it.
...
Voila, packaging the whole project neatly in a docker container will solve all our problems right?

Right?!

![A meme depicting the "works on my docker" issue 600x](/blog/talking-to-water/works-on-my-docker.webp)
...
This could be to a machine on the other side of the room, building, city or even world. Or in our case, to a docker container.
