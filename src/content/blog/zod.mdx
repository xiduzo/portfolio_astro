...
```typescript
const input = { id: '123', type: false, amount: 42 }

const example = GrowthCycle.parse(input) // Throws a `ZodError`
// or
const { success, data, error } = GrowthCycle.safeParse(input) // Returns a `ZodError`
```
...
const example = new GrowthCycle(input) // [!code error]
