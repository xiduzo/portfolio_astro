...
You can easily spot those lies in source code whenever you see an `as LAZY_LIE`.
...
For a client I am working for, [Growy](https://www.growy.nl/), most of the data is coming from unstructured data sources.

Growy is highly automizing vertical farming and is relying on a hundreds of IoT devices which are sending and receiving data to and from the cloud.
...
For DynamoDB queries we had functions similar to:
...
While all interfaces, and static types are correct, the runtime data could still be (and sometimes was) wrong.

![IoT devices](/blog/zod/iot-devices.png "IoT devices slowly being added to the platform")

While most of the systems worked fine, the platform was only tested in a controlled environment with simulated IoT devices.
...
In order to prevent services from crashing and data from being stored incorrectly, we added runtime validation to the data objects.
...
But as time went on, received data was more and more in line with the expected format. We have better discussions between the teams about the data format and the interfaces as faulty data will not be accepted anymore.

The platform is more stable, and the data is more reliable.
