...
We can enhance the interface by adding a runtime validation library like [Zod](https://zod.dev/)[^1]. This requires some initial extra typing, but it pays off in terms of safer code and new type-safe superpowers inside your IDE.
...
Instead of defining a type, we define a schema with `Zod` and infer the interface from it.
...
```typescript
export function getData(): ApiData | null {
...
Growy is revolutionizing vertical farming by heavily relying on (hundreds of) IoT devices sending and receiving data to/from the cloud. We receive data from two primary sources: DynamoDB and MQTT messages.
...
function getRecord(id: string) {
  ...
  return GrowthCycle.fromDynamoRecord(Item);
}
```

And for MQTT messages, we've created handlers that parse data as follows:
...
mqqClient.onMessage(message => {
  ...
  const example = new GrowthCycle(data.id, data.type, data.amount);

  // ... do something with the data
})
```

But, you might have noticed some `as LAZY_LIE`s in the code.
...
const example = GrowthCycle.parse(input)
// or
const { success, data, error } = GrowthCycle.safeParse(input)
```
